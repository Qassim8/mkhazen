...
export default function CreateOrderClient({ products, suppliers }: Props) {
  ...
  const [selectedPurchaseType, setSelectedPurchaseType] =
    useState<PurchaseOrderType>("WORKFLOW");

  // حالة التحكم بالبحث عن المنتجات المتغيرة (Combobox State)
  ...
  // إغلاق القائمة عند النقر خارج صندوق البحث
  useEffect(() => {
  ...
  const onSubmit = async (data: CreatePurchaseOrderInput) => {
    ...
    try {
      ...
      toast.success(
        selectedPurchaseType === "DIRECT"
          ? "تم تنفيذ الشراء المباشر بنجاح"
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-end gap-3 border-t border-gray-100 pt-6 mt-6">
        ...
        <button
          type="submit"
          disabled={isSubmitting}
          onClick={() => setSelectedPurchaseType("WORKFLOW")}
        ...
        >
          حفظ كأمر شراء (Workflow)
        </button>

        <button
          type="submit"
          disabled={isSubmitting}
          onClick={() => setSelectedPurchaseType("DIRECT")}
        ...
        >
          {isSubmitting ? "جاري التنفيذ..." : "شراء مباشر (Direct)"}
        </button>
      </div>
    </form>
  );
